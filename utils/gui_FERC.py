...
class Stimuli:
    ...
    def __init__(self, window, frequency=10, phase=0, amplitude=1., wave_type='sin', duration=5, fps=60, base_pos=(0, 0), height=100, width=100):
        self.window = window
        self.freq = frequency
        self.phase = phase
        self.amp = amplitude
    ...
    def _get_wave_func(self, duration, fps):
        ...
        if (self.wave_type == 'sin') or (self.wave_type == 'square'):
            frame_index = np.arange(0, length, 2)
            wave = 2*(1 + wave_func(2 * np.pi * self.freq * (frame_index / fps) + (self.phase * np.pi)))
